    ...
    with open('kiinteisto_debug.log', 'a', encoding='utf-8') as f:
        ...
        for kiinteistotunnus in kiinteistotunnukset:
            ...
            for rakennustiedot_cluster in clustered_rakennustiedot:
                ...
                # 3. Jaa muut kuin yhden asoy:n klusterit omistajien mukaan
                if tracked_buildings:
                    f.write("\nEi yhden asoy:n klusteri, jaetaan omistajien mukaan\n")
                ...
                while remaining_ids:
                    ...
                    # Luo rakennusryhmä kaikista rakennuksista
                    rakennustiedot_group = {
                        dvv_rakennustiedot[id] 
...
def create_perusmaksurekisteri_kohteet(
    ...
    buildings_by_customer = defaultdict(set)
    ...
    for index, row in enumerate(sheet.values):
        ...
        if rakennus.rakennusluokka_2018 in ALLOWED_TYPES_2018:
            buildings_by_customer[asiakasnumero].add(dvv_rakennustiedot_by_prt[prt])
            buildings_found += 1
    ...
    for asiakasnumero, buildings in buildings_by_customer.items():
        if buildings:  # Lisää vain jos ryhmässä on rakennuksia
            building_sets.append(buildings)
            valid_sets += 1
