...
def test_import_dvv_kohteet(engine, datadir):
    with Session(engine) as session:
        ...
        import_data(
            datadir + "/kuljetus1", "LSJ", "1.1.2022", "31.12.2022"
        )
        session.expire_all()
        # DEBUG: print all kohde names and sopimukset
        all_kohteet = session.query(Kohde.id, Kohde.nimi, Kohde.loppupvm).all()
        print("\n=== DEBUG: Kaikki kohteet ===")
        for k in all_kohteet:
            print(f"  id={k[0]}, nimi={k[1]!r}, loppupvm={k[2]}")
        all_sopimukset = session.query(Sopimus.id, Sopimus.kohde_id, Jatetyyppi.selite).join(Jatetyyppi, Sopimus.jatetyyppi_id == Jatetyyppi.id).all()
        print("=== DEBUG: Kaikki sopimukset ===")
        for s in all_sopimukset:
            print(f"  id={s[0]}, kohde_id={s[1]}, jatelaji={s[2]!r}")
        all_rakennus_kohde = session.query(KohteenRakennukset.kohde_id, Rakennus.prt).join(Rakennus, KohteenRakennukset.rakennus_id == Rakennus.id).all()
        print("=== DEBUG: Kohteiden rakennukset (PRT) ===")
        for r in all_rakennus_kohde:
            print(f"  kohde_id={r[0]}, prt={r[1]!r}")
        print("=== END DEBUG ===\n")
        ...
        import_data(
            datadir + "/kuljetus2", "LSJ", "1.1.2023", "31.12.2023"
        )
        session.expire_all()
...
def test_update_dvv_kohteet(engine, datadir):
    ...
    if in_container:
        update_test_db_command = str(scripts_dir / "update_database_container.sh")
    elif platform.system() == 'Windows':
        update_test_db_command = str(scripts_dir / "update_database.bat")
    else:
        update_test_db_command = str(scripts_dir / "update_database.sh")
    ...
    with Session(engine) as session:
        ...
        import_data(
            datadir + "/kuljetus3", "LSJ", "1.4.2023", "30.6.2023"
        )
        session.expire_all()
        ...
        import_data(Path(fixed_folder), "LSJ", "1.1.2023", "31.3.2023")
        session.expire_all()
