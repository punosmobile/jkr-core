#!/bin/bash
# update_database_container.sh
# Päivittää testikannan DVV_update.xlsx-tiedostolla kun ajetaan Docker-kontainerissa.
...
INPUT_FILE="$TESTS_DIR/data/test_data_import/DVV_update.xlsx"

echo "Tuodaan rakennukset (DVV päivitys)..."
ogr2ogr -f PostgreSQL -overwrite -progress \
  "$OGR_DSN" \
  -nln rakennus "$INPUT_FILE" "R1 rakennus"

echo "Tuodaan osoitteet (DVV päivitys)..."
ogr2ogr -f PostgreSQL -overwrite -progress \
  "$OGR_DSN" \
  -nln osoite "$INPUT_FILE" "R3 osoite"

echo "Tuodaan omistajat (DVV päivitys)..."
ogr2ogr -f PostgreSQL -overwrite -progress \
  "$OGR_DSN" \
  -nln omistaja "$INPUT_FILE" "R4 omistaja"

echo "Tuodaan asukkaat (DVV päivitys)..."
ogr2ogr -f PostgreSQL -overwrite -progress \
  "$OGR_DSN" \
...
psql -h "$JKR_DB_HOST" -p "$JKR_DB_PORT" -d "$JKR_DB" -U "$JKR_USER" \
  -v formatted_date=20230131 -f "$PROJECT_DIR/scripts/import_dvv.sql"

echo "Testikannan päivitys valmis."
