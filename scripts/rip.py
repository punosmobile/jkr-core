...
def process_csv(file_path: Path, must_contain: set[str], must_contain_wild: set[str]) -> None:
    ...
    try:
        ...
        for line in content[1:]:  # Skip header when checking
            if (any(word in line.lower() for word in must_contain) or 
                any(any(word.startswith(prefix) for prefix in must_contain_wild) for word in line)):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            ...
            for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
                ...
                if (any(word in row_text for word in must_contain) or 
                    any(any(word.startswith(prefix) for prefix in must_contain_wild) for word in row_text)):
